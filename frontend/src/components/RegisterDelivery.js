...
const RegisterDelivery = () => {
    ...
    const errors = {
        ...
        address: validators.kanoAddress(form.address, true),
    };
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await apiClient.post('/auth/register-delivery', form);
            const succMsg = 'Registration submitted! Your application is pending admin approval.';
            setMessage(succMsg);
        ...
        <main className="auth-page min-vh-100 d-flex align-items-center justify-content-center bg-light py-5">
            <div className="auth-card auth-card--wide card border-0 shadow-sm custom-card p-4">
                <div className="text-center mb-3">
                    ...
                    <h3 className="fw-bold mt-2">Become a Delivery Partner</h3>
                    <p className="text-muted small">Deliver packages in Kano & earn 65% commission on every completed order</p>
                </div>

                <div className="alert alert-warning py-2 small d-flex align-items-center gap-2">
                    <span>⚠️</span> Note: Partner accounts require quick approval from Admin before accepting orders.
                </div>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-3">
                        ...
                        <input name="full_name" className={fieldClassName('form-control', form.full_name, touched.full_name, errors.full_name)} placeholder="Ahmadu Bello" value={form.full_name} onChange={handleChange} onBlur={() => touch('full_name')} aria-invalid={Boolean(touched.full_name && errors.full_name)} required />
                    ...
                    <div className="mb-3">
                        <label className="form-label small fw-semibold text-muted">Operating Base / Address in Kano</label>
                        <textarea name="address" className={fieldClassName('form-control', form.address, touched.address, errors.address)} rows="2" placeholder="Nasarawa, Kano" value={form.address} onChange={handleChange} onBlur={() => touch('address')} aria-invalid={Boolean(touched.address && errors.address)} required></textarea>
                        <div className="form-text">Delivery partners must operate from Kano.</div>
                        <ValidationFeedback touched={touched.address} error={errors.address} validMessage="Kano operating base recognised." />
                    </div>
                    <button className="btn btn-warning text-dark w-100 py-2 fw-semibold d-flex align-items-center justify-content-center" disabled={loading}>
                        {loading ? (
                            <>
                ...
                <div className="text-center mt-4 border-top pt-3">
                    <p className="small text-muted mb-0">Already registered? <Link to="/login" className="fw-semibold">Sign In</Link></p>
                </div>
            </div>
        </main>
    );
